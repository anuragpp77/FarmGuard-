...
# TWILIO_TO_NUM      = os.getenv("TWILIO_TO_NUM")

# print('SID:', TWILIO_ACCOUNT_SID)
# print('Token:', TWILIO_AUTH_TOKEN[:6], '...')
# print('From:', TWILIO_FROM_NUM)
# print('To:', TWILIO_TO_NUM)

# client = Client(TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN)
# msg = client.messages.create(body='Test from FarmGuard', from_=TWILIO_FROM_NUM, to=TWILIO_TO_NUM)
# print('SUCCESS! SID:', msg.sid)


# # --- PAGE CONFIGURATION ---
# st.set_page_config(page_title="FarmGuard", page_icon="🛡️", layout="wide", initial_sidebar_state="expanded")

# # --- CUSTOM CSS INJECTION ---
# st.markdown("""
# <style>
#     /* SQUASH STREAMLIT'S DEFAULT TOP PADDING */
...
#     """Sends a WhatsApp alert via Twilio using the background credentials."""
#     try:
#         client = Client(TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN)
#         message = client.messages.create(
#             body=f"⚠️ FarmGuard ALERT: {animal.upper()} detected near the perimeter!",
...
#     🛡️ FarmGuard Wildlife Detection System
# </div>
# """, unsafe_allow_html=True)
...
#         {"type": "info", "title": "System initialized", "sub": "Ready for monitoring", "time": datetime.now().strftime('%H:%M:%S')}
#     ]
...
#         <div style="width: 100%; height: 450px; background-color: #1a1e23; border-radius: 8px; display: flex; align-items: center; justify-content: center; color: #4a5568; font-size: 1.2rem; border: 1px solid #2d343f;">
...
#              "type": "info", "title": "Monitoring started", "sub": "YOLOv8 initialized", "time": datetime.now().strftime('%H:%M:%S')
#          })
#          render_logs()
...
#                 if label in animal_classes:
#                     cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), (180, 190, 130), 2)
#                     text = f"{label.capitalize()} - {int(conf*100)}% Conf."
#                     cv2.rectangle(annotated_frame, (x1, y1-25), (x1 + len(text)*10, y1), (180, 190, 130), -1)
#                     cv2.putText(annotated_frame, text, (x1+5, y1-8), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)
...
#             if current_time - st.session_state['last_alert_time'] >= alert_interval:
...
#                 log_type = "warning" if animal in ["zebra", "buffalo"] else "danger"
                
#                 # Log the detection
#                 st.session_state['logs'].insert(0, {
#                     "type": log_type,
#                     "title": f"{animal.capitalize()} detected",
#                     "sub": f"Confidence {int(conf*100)}% · {video_source.split('.')[0].capitalize()}",
#                     "time": time_str
#                 })
                
#                 # --- ACTUAL WHATSAPP TRIGGER ---
#                 send_whatsapp_alert(animal)
                
#                 # Log the alert dispatch
#                 st.session_state['logs'].insert(0, {
#                     "type": "info",
#                     "title": "Alert sent · Twilio",
#                     "sub": f"SMS delivered to {TWILIO_TO_NUM}",
#                     "time": time_str
#                 })
